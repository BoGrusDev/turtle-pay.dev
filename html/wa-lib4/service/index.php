...
class WebApi {
    ...
    */
    ...
    private function actionRequest($data) {
        ...
        */
        /*
            Check if Pep, and if a KYC is needed
            If a KYC is needed return true 
        */

        $replyPep = $this->pepCheck($data->personal_id_number);
        if ( $replyPep->code == '6') {
            echo json_encode($replyPep);
            return;
        }

        /*
            if ($this->pepCheck($data->personal_id_number)) {
    ...
    private function actionProgress($data) {
       ...
       */
        ...
        //
        // Testing
        //$paramComplete->signature = 'Test signature';
        //$paramComplete->ocspResponse = "Test ocsp";
        ...
        if ($dataParam->uc == 'n') {
            ...
            // echo json_encode($paramComplete); die('');

            // SPEC
            //$paramComplete->spec = $dataParam->spec;
            $result =  json_decode($this->_RestApiCall(json_encode($paramComplete)));
            // print_r($result); 
            if ($result->code == '1') {
                echo $this->completeAll($result, $dataParam->event_item_id, $dataParam->email, $resWaLog);
            }
            else {
                $replyObj = new StdClass();
                $replyObj->code = "0";
                $replyObj->denied_code = "complete-rejected-2";
                echo json_encode($replyObj);
                die('');
            }
        }
            ...
            if (json_decode($result)) {
                ...
                $replyObj = new StdClass();
                $replyObj->code = "0";
                $replyObj->denied_code = "complete-rejected-3";
                echo json_encode($replyObj);
                die('');
            }
            */
    ...
    private function _RestApiCall($param, $url = 'default') {
        // 2022-03-25
    ...
    private function completeAll($resultObj, $eventItemId, $email, $waLogResult) {
	...
	private function actionSpar($data) {
		...
		// Prod
		curl_setopt($ch, CURLOPT_SSLCERT, 'spar-2024.pem');
        curl_setopt($ch, CURLOPT_SSLCERTPASSWD, '4347202132563555');
    ...
    private function pepCheck($personalIdNumber) {
        ...
        $res = $this->_RestApiCall(json_encode($param), KAPI_URL);
